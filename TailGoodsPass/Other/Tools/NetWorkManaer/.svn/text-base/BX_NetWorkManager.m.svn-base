...
#import "BX_NetWorkManager.h"
#import "DESEncrypt.h"
#import "MBProgressHUD+Add.h"


@implementation BX_NetWorkManager

+ (BX_NetWorkManager *)shareManager {
    static BX_NetWorkManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[BX_NetWorkManager alloc] init];
    });
    return manager;
}
...
+ (void)requestForGETWithUrl:(NSString *)url parameter:(NSDictionary *)dic success:(void(^)(id responseObject))success failure:(void(^)(NSError *error))conError {
    ...
    //开始网络请求
    [manager GET:[NSString stringWithFormat:@"%@%@", kServiceURL, url] parameters:dic progress:^(NSProgress * _Nonnull downloadProgress) {
...
//    [manager.requestSerializer setValue:@"text/plain" forHTTPHeaderField:@"Content-Type"];
    [manager POST:[NSString stringWithFormat:@"%@%@", kServiceURL, url] parameters:dic progress:^(NSProgress * _Nonnull uploadProgress) {
        //上传数据的进度
...
+ (NSString *)arrayToString:(NSArray *)arr {
    ...
    for (id obj in arr) {
        if ([obj isKindOfClass:[NSDictionary class]]) {
            [string appendString:[BX_NetWorkManager dicToString:obj]];
        } else if ([obj isKindOfClass:[NSString class]]) {
            [string appendString:[NSString stringWithFormat:@"\"%@\"", obj]];
        } else if ([obj isKindOfClass:[NSArray class]]) {
            [string appendString:[BX_NetWorkManager arrayToString:obj]];
        } else {
            [string appendString:[NSString stringWithFormat:@"%@", obj]];
        }
...
+ (NSString *)dicToString:(NSDictionary *)dic{
    ...
    for (NSString *keyStr in dic.allKeys) {
        ...
        if ([[dic valueForKey:keyStr] isKindOfClass:[NSArray class]]) {
            [string appendString:[BX_NetWorkManager arrayToString:[dic valueForKey:keyStr]]];
        }else if ([[dic valueForKey:keyStr] isKindOfClass:[NSDictionary class]]) {
            [string appendString:[BX_NetWorkManager dicToString:[dic valueForKey:keyStr]]];
